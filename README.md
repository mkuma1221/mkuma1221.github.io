...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // 2. Create the container div for tsParticles
  const particlesDiv = document.createElement('div');
  particlesDiv.id = 'tsparticles';
  
  // Insert it as the first child inside .page-header
  header.insertBefore(particlesDiv, header.firstChild);

  // 3. Initialize tsParticles with Starfield + Twinkle + Interaction
  tsParticles.load("tsparticles", {
    fullScreen: { enable: false }, // Restrict particles to banner container
    fpsLimit: 60,
    particles: {
      number: {
      ...
      shape: { type: "circle" },
      opacity: {
        value: { min: 0.2, max: 0.9 }, // Varying star brightness
        animation: {
          enable: true,
      ...
      size: {
        value: { min: 0.8, max: 2.2 }, // Tiny realistic stars
        animation: {
          enable: true,
          speed: 1,
          minimumValue: 0.5,
    ...
    interactivity: {
      ...
      events: {
        onHover: {
          enable: true,
          mode: "grab" // Connects stars with subtle lines on hover
        },
        onClick: {
          enable: true,
          mode: "push" // Spawns extra stars on click
        },
        resize: true
      },
      modes: {
        grab: {
          distance: 140,
          links: { opacity: 0.35, color: "#ffffff" }
        },
        push: { quantity: 4 }
      }
    },
